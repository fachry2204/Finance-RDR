...
import { Bell, Send, Users, AlertCircle, CheckCircle, Info, User, Trash2, Clock, List, RefreshCw } from 'lucide-react';
        ...
        <div className="space-y-6">
            ...
            <div className="flex gap-2 border-b border-slate-200 mb-6">
                <button
                    ...
                    className={`pb-3 px-4 text-sm font-medium transition-colors relative ${activeTab === 'create' ? 'text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
                >
                    <div className="flex items-center gap-2">
                        <Send size={16} /> Buat Pesan
                    </div>
                    {activeTab === 'create' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 rounded-t-full"></div>}
                </button>
                <button
                    onClick={() => setActiveTab('history')}
                    className={`pb-3 px-4 text-sm font-medium transition-colors relative ${activeTab === 'history' ? 'text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
                >
                    <div className="flex items-center gap-2">
                        <Clock size={16} /> Riwayat Pesan
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                    <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                        <h2 className="font-bold text-lg flex items-center gap-2">
                            <List size={20} className="text-blue-600" /> Riwayat Notifikasi Terkirim
                        </h2>
                        <button 
                    ...
                    {historyLoading ? (
                         <div className="p-12 text-center text-slate-500">
                            <RefreshCw size={32} className="mx-auto mb-3 animate-spin text-blue-300" />
                            <p>Memuat riwayat...</p>
                        </div>
                    ) : history.length === 0 ? (
                        <div className="p-12 text-center text-slate-500">
                            ...
                            <p className="text-lg font-medium text-slate-700">Belum ada riwayat pesan</p>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {history.map((item) => (
                                        <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="p-4 text-sm text-slate-600 max-w-md truncate" title={item.message}>
                                                {item.message}
                                            </td>
                                            <td className="p-4">
